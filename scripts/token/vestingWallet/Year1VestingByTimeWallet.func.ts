...
async function transferZOICToVestingWallet() {
  ...
  const zoicToken = await getContract('TokenZOIC', ContractDeployAddress.TokenZOIC);

  const year1VestingByTimeWallet = await getContract(
    'VestingByTimeWallet',
    ContractDeployAddress.Year1VestingByTimeWallet
  );
  const balance = await zoicToken.balanceOf(ContractDeployAddress.Year1VestingByTimeWallet);
  if (balance.gt(ethers.utils.parseEther('0'))) {
    console.log('the first year vesting wallet has withdrawn, now balance is', balance);
    return;
  }

  const amount = ethers.utils.parseEther(RELEASE_STEP[0].toString());

  const zoicTokenCoffer = await getContract('TokenCoffer', ContractDeployAddress.ZOICTokenCoffer);

  const [owned] = await ethers.getSigners();
  //grant minter role to default caller
  const tx = await zoicTokenCoffer.grantRole(ethers.utils.id('WITHDRAW'), owned.address);
  const receipt = await tx.wait();

  const txWithdraw = await zoicTokenCoffer
    .connect(owned)
    .withdrawERC20(ContractDeployAddress.TokenZOIC, year1VestingByTimeWallet.address, amount);
