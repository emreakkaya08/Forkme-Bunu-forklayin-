...
describe('RedeemU', () => {
  ...
  it('Should redeem ERC20 tokens correctly', async () => {
    ...
    await mockUSDT
      .connect(owner)
      .transfer(treasury.address, ethers.utils.parseEther('1000'));
    ...
    await mockXToken
      .connect(addr1)
      .approve(redeemU.address, ethers.utils.parseEther('9'));
    await mockYToken
      .connect(addr1)
    ...
    await redeemU
      .connect(addr1)
      .redeemERC20(ethers.utils.parseEther('9'), mockUSDT.address);

    //打印销毁后 X 和 Y 代币余额
    console.log(
      'burn finshed X balance: ',
      await mockXToken.balanceOf(addr1.address)
    );
    console.log(
      'burn finshed Y balance: ',
      await mockYToken.balanceOf(addr1.address)
    );

    // 检查X和Y代币已被销毁
    expect(await mockXToken.balanceOf(addr1.address)).to.equal(
      ethers.utils.parseEther('91')
    );
    expect(await mockYToken.balanceOf(addr1.address)).to.equal(
