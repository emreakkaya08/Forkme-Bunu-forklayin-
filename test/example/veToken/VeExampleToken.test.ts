...
describe('VeExampleToken', () => {
  ...
  beforeEach(async () => {
    ...
    veExampleToken = await upgrades.deployProxy(VeExampleToken, [exampleToken.address]);
  ...
  describe('createLock', () => {
    ...
    it('should create lock', async () => {
      ...
      expect(await exampleToken.balanceOf(veExampleToken.address)).to.equal(amount);
  ...
  describe('calculateUnlockTime', () => {
    ...
    valuesToTest.forEach(([unlockTime, expected]) => {
      it(`should in ${unlockTime} return ${expected}`, async () => {
        expect(await veExampleToken.calculateUnlockTime(unlockTime)).to.equal(expected);
      });
    });
  });
  ...
  describe('withdraw', () => {
    ...
    it('should failed: not unlock', async () => {
      ...
      //withdraw
      await expect(veExampleToken.withdraw()).rejectedWith('VeToken: locked balance is not unlock');
    });
  });
  ...
  describe('increaseAmount', () => {
    ...
    it('should success', async () => {
      ...
      //check token balance
      expect(await exampleToken.balanceOf(veExampleToken.address)).to.equal(totalAmount);
  ...
  describe('totalSupply', () => {
    ...
    it('should 2 users create lock', async () => {
      ...
      await veExampleToken.connect(owner).createLock(amount, BigNumber.from(lockTime));

      const balanceOfOwner = await veExampleToken.balanceOfAtTime(
        owner.address,
        block.timestamp + week
      );
      let totalSupply = await veExampleToken.totalSupplyAtTime(block.timestamp + week);

      expect(balanceOfOwner).to.equal(totalSupply);

      //addr1 create lock
      await exampleToken.connect(addr1).approve(veExampleToken.address, amount);
      await veExampleToken.connect(addr1).createLock(amount, BigNumber.from(lockTime));

      const balanceOfAddr1 = await veExampleToken.balanceOfAtTime(
        addr1.address,
        block.timestamp + week
      );
      totalSupply = await veExampleToken.totalSupplyAtTime(block.timestamp + week);
